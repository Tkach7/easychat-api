    ...
    new Promise<Connection>((resolve, reject) => {
      rethinkdb.connect(
        ...
        (error: ReqlDriverError, conn: Connection) => {
          ...
          db.createDb(conn);
          Promise.all([USER_TABLE.init(conn), TOPIC_TABLE.init(conn)]).then(() => resolve(conn));
        }
      );
    }),
  createDb: (conn: Connection) => {
    (rethinkdb.dbList() as any)
      ...
      .do((databaseExists: any) =>
        rethinkdb.branch(databaseExists, {dbs_created: 0} as any, rethinkdb.dbCreate(config.get('db_name')) as any)
      )
      .run(conn);
  }
};
